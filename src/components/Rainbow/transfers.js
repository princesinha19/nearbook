...
export async function track(transferRaw, callback) {
    let id;
...
function initiateApproval(amount) {
    return new Promise((resolve, reject) => {
        window.erc20.methods
            .approve(config.development.ethLockerAddress, amount).send()
        ...
        if (approvalReceipt) {
            if (approvalReceipt.status) {
                ...
                const error = await getRevertReason(
                    transfer.approvalHash, config.development.ethNetwork
                )
                transfer = update(transfer, {
                    status: COMPLETE,
                    outcome: FAILED,
    ...
    if (transfer.status === INITIATED_LOCK) {
        ...
        if (lockReceipt) {
            if (lockReceipt.status) {
                ...
                const error = await getRevertReason(
                    transfer.lockHash, config.development.ethNetwork
                )
                transfer = update(transfer, {
                    status: COMPLETE,
                    outcome: FAILED,
